...
@Table(name = "V_COURSE")
@NamedQueries({
        @NamedQuery(name = "VCourse.findAllSince", query = "select a from VCourse a where a.lastChanged > :since"),
        @NamedQuery(name = "VCourse.findAll", query = "select a from VCourse a") })
public final class VCourseDto {
  @Id
  private String id;
  private String name;
  ...
  public static final Function<VCourseDto, VCourse> toDomain = new Function<VCourseDto, VCourse>() {
    @Override
    public VCourse apply(VCourseDto dto) {
  ...
  public static final Finder<VCourseDto> finder = new Finder<VCourseDto>("VCourse");
}
