describe('Events controller', function () {
    ...
    describe('#delete', function () {
        ...
        it('reloads events after deletion', function () {
            $httpBackend.expectGET('/edit-ng/resources/events/filters.json').respond('[]');
            $httpBackend.expectGET('/edit-ng/resources/PUBLICATION.CHANNEL.LABELS.json').respond('{}');
            $httpBackend.expectDELETE('/edit-ng/event/12').respond('12');
            $httpBackend.expectGET('/edit-ng/event/events.json?limit=10&offset=0').respond(JSON.stringify(getJSONFixture('edit-ng/event/events.json')));
