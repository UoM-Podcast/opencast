describe('ACLs API Resource', function () {
    ...
    describe('#query', function () {
        ...
        it('calls the acls service', function () {
            $httpBackend.expectGET('/edit-ng/acl/acls.json').respond(JSON.stringify(getJSONFixture('edit-ng/acl/acls.json')));
            AclsResource.query();
            $httpBackend.flush();
        });

        it('flattens the JSON data received', function () {
            $httpBackend.whenGET('/edit-ng/acl/acls.json').respond(JSON.stringify(getJSONFixture('edit-ng/acl/acls.json')));
