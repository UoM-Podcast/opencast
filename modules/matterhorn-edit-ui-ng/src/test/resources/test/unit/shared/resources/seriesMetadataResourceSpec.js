describe('Series Metadata API Resource', function () {
    ...
    describe('#get', function () {
        it('provides nested JSON data', function () {
            ...
            $httpBackend.expectGET('/edit-ng/series/40518/metadata.json').respond(JSON.stringify(metadataResponse));
            result = SeriesMetadataResource.get({ id: 40518 });
            $httpBackend.flush();
    ...
    describe('#save', function () {
        it('sends an array of metadata', function () {
            ...
            $httpBackend.expectPUT('/edit-ng/series/40518/metadata', function (data) {
                expect($.deparam(data)).toEqual({
                    metadata: JSON.stringify([metadataRequest])
                });
                return true;
            }).respond(200);
            SeriesMetadataResource.save({ id: '40518' }, metadataRequest);
            $httpBackend.flush();
        });
        ...
        it('handles empty requests gracefully', function () {
            $httpBackend.expectPUT('/edit-ng/series/8/metadata').respond(200);
