describe('Group API Resource', function () {
    ...
    describe('#get', function () {
        ...
        it('queries the group API', function () {
            $httpBackend.expectGET('/edit-ng/groups/it_team').respond(JSON.stringify(sampleJSON));
            GroupResource.get({ id: 'it_team' });
            $httpBackend.flush();
        });

        it('returns the parsed JSON', function () {
            $httpBackend.whenGET('/edit-ng/groups/it_team').respond(JSON.stringify(sampleJSON));
