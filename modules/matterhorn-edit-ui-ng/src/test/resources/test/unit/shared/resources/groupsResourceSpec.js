describe('Groups API Resource', function () {
    ...
    describe('#query', function () {
        ...
        it('calls the groups service', function () {
            $httpBackend.expectGET('/edit-ng/groups/groups.json').respond(JSON.stringify(sampleJSON));
            GroupsResource.query();
            $httpBackend.flush();
        });

        it('flattens the JSON data received', function () {
            $httpBackend.whenGET('/edit-ng/groups/groups.json').respond(JSON.stringify(sampleJSON));
        ...
        it('handles payloads with zero items', function () {
            $httpBackend.whenGET('/edit-ng/groups/groups.json').respond(JSON.stringify(
                {
                  count: 0,
                  limit: 0,
                  offset: 0,
