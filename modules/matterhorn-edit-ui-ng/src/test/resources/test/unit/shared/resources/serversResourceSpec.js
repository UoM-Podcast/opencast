describe('Servers API Resource', function () {
    ...
    describe('#query', function () {
        ...
        it('calls the servers service', function () {
            $httpBackend.expectGET('/edit-ng/server/servers.json').respond(JSON.stringify(sampleJSON));
            ServersResource.query();
            $httpBackend.flush();
        });

        it('flattens the JSON data received', function () {
            $httpBackend.whenGET('/edit-ng/server/servers.json').respond(JSON.stringify(sampleJSON));
