describe('Jobs API Resource', function () {
    ...
    describe('#query', function () {
        ...
        it('calls the jobs service', function () {
            $httpBackend.expectGET('/edit-ng/job/jobs.json').respond(JSON.stringify(sampleJSON));
            JobsResource.query();
            $httpBackend.flush();
        });

        it('flattens the JSON data received', function () {
            $httpBackend.whenGET('/edit-ng/job/jobs.json').respond(JSON.stringify(sampleJSON));
