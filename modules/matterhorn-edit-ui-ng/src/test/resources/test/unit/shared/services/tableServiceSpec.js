describe('Table', function () {
    ...
    describe('#fetch', function () {
        ...
        it('updates the table data from the API', function () {
            $httpBackend.expectGET('/edit-ng/users/users.json?limit=10&offset=0')
                .respond(JSON.stringify(getJSONFixture('edit-ng/users/users.json')));
            Table.fetch();
            $httpBackend.flush();
        ...
        describe('with a table filter', function () {
            ...
            it('constructs a valid query string', function () {
                $httpBackend.expectGET('/edit-ng/users/users.json?filter=username:admin&limit=10&offset=0')
                    .respond(JSON.stringify(getJSONFixture('edit-ng/users/users.json')));
                Table.fetch();
                $httpBackend.flush();
            });
    ...
    describe('#sortBy', function () {
        beforeEach(function () {
            ...
            $httpBackend.whenGET('/edit-ng/users/users.json?limit=10&offset=0')
                .respond(JSON.stringify(getJSONFixture('edit-ng/users/users.json')));
            Table.fetch();
            $httpBackend.flush();
        });
        ...
        it('sets the sort params', function () {
            $httpBackend.whenGET('/edit-ng/users/users.json?limit=10&offset=0&sort=email:ASC')
                .respond(JSON.stringify(getJSONFixture('edit-ng/users/users.json')));
        ...
        it('stores the filters', function () {
            spyOn(Storage, 'put');
            $httpBackend.whenGET('/edit-ng/users/users.json?limit=10&offset=0&sort=email:DESC')
                .respond(JSON.stringify(getJSONFixture('edit-ng/users/users.json')));
            Table.sortBy({ name: 'email' });
            $httpBackend.flush();
