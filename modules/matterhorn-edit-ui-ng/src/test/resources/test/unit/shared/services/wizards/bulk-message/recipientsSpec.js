describe('Recipients Step for the Bulk Message Wizard', function () {
    ...
    beforeEach(function () {
        $httpBackend.whenGET('/edit-ng/capture-agents/agents.json')
            .respond('{"results":[]}');
    });
    ...
    describe('retrieving recipients', function () {
        ...
        describe('for events', function () {
            beforeEach(function () {
                ...
                $httpBackend.expectGET('/edit-ng/event/events/recipients?eventIds=')
                    .respond('{}');
                $httpBackend.flush();
            });
            ...
            it('fetches recipients for selected table rows', function () {
                ...
                $httpBackend.expectGET('/edit-ng/event/events/recipients?eventIds=5,9')
                    .respond(JSON.stringify(getJSONFixture('edit-ng/event/events/recipients')));
                $httpBackend.flush();
        ...
        describe('for series', function () {
            ...
            it('fetches recipients for selected table rows', function () {
                BulkMessageRecipients.reset();
                $httpBackend.expectGET('/edit-ng/series/series/recipients?seriesIds=5,9')
                    .respond(JSON.stringify(getJSONFixture('edit-ng/event/events/recipients')));
                $httpBackend.flush();
