...
//
exports.NewUserBlacklistPage = function () {
    this.get = function () {
        browser.get('http://localhost:9007/#/users/userblacklists');
    };

    this.newBlacklistButton     = element(by.css('button[data-open-modal="user-blacklist-modal"]'));
    this.wizard                 = {};
    this.wizard.header          = element(by.id('user-blacklist-modal'));
    this.wizard.activeStep      = element(by.css('nav.step-by-step a.active'));
    this.wizard.nextButton      = element(by.css('edit-ng-wizard a.submit'));
