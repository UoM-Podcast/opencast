...
import org.opencastproject.schema.OcDublinCoreUtil.OcDublinCoreConversion;
...
public final class Schema {
  ...
  public static final String DUBLINCORE_SERIES = "dublincore_series";
  ...
  public static void fill(final SolrInputDocument doc, final FieldCollector fields) {
    final FieldCollector copyToDoc = new FieldCollector() {
      ...
      public Option<String> getOrganization() {
        for (String a : fields.getOrganization())
          setOrganization(doc, a);
        return null;
      }

      @Override
      public Option<String> getDcId() {
        for (String a : fields.getDcId())
          setDcId(doc, a);
        return null;
      }
      ...
      public Option<Date> getDcCreated() {
        for (Date a : fields.getDcCreated())
          setDcCreated(doc, a);
        return null;
      }
      ...
      public Option<String> getDcLanguage() {
        for (String a : fields.getDcLanguage())
          setDcLanguage(doc, a);
        return null;
      }
      ...
      public Option<String> getDcIsPartOf() {
        for (String a : fields.getDcIsPartOf())
          setDcIsPartOf(doc, a);
        return null;
      }
      ...
      public Option<String> getDcReplaces() {
        for (String a : fields.getDcReplaces())
          setDcReplaces(doc, a);
        return null;
      }
      ...
      public Option<Date> getDcAvailableFrom() {
        for (Date a : fields.getDcAvailableFrom())
          setDcAvailableFrom(doc, a);
        return null;
      }

      @Override
      public Option<Date> getDcAvailableTo() {
        for (Date a : fields.getDcAvailableTo())
          setDcAvailableTo(doc, a);
        return null;
      }
      ...
      public List<DField<String>> getSeriesDcTitle() {
        for (DField<String> v : fields.getSeriesDcTitle())
          setSeriesDcTitle(doc, v);
        return null;
      }

      @Override
      public Option<String> getOcMediapackage() {
        for (String a : fields.getOcMediapackage())
          setOcMediapackage(doc, a);
        return null;
      }

      @Override
      public Option<String> getOcAcl() {
        for (String a : fields.getOcAcl())
          setOcAcl(doc, a);
        return null;
      }
      ...
      public Option<String> getOcKeywords() {
        for (String a : fields.getOcKeywords())
          setOcKeywords(doc, a);
        return null;
      }
      ...
      public Option<String> getOcCover() {
        for (String a : fields.getOcCover())
          setOcCover(doc, a);
        return null;
      }
      ...
      public Option<Date> getOcModified() {
        for (Date a : fields.getOcModified())
          setOcTimestamp(doc, a);
        return null;
      }

      @Override
      public Option<Boolean> getOcDeleted() {
        for (Boolean a : fields.getOcDeleted())
          setOcDeleted(doc, a);
        return null;
      }
      ...
      public Option<String> getOcElementtags() {
        for (String a : fields.getOcElementtags())
          setOcElementtags(doc, a);
        return null;
      }

      @Override
      public Option<String> getOcElementflavors() {
        for (String a : fields.getOcElementflavors())
          setOcElementflavors(doc, a);
        return null;
      }

      @Override
      public Option<Version> getOcVersion() {
        for (Version a : fields.getOcVersion())
          setOcVersion(doc, a);
        return null;
      }
      ...
      public Option<Boolean> getOcLatestVersion() {
        for (Boolean a : fields.getOcLatestVersion())
          setOcLatestVersion(doc, a);
        return null;
      }
  ...
  public static boolean getOcDeleted(SolrDocument doc) {
    return mkBoolean(doc.get(OC_DELETED));
  }

  public static void setOcDeleted(SolrInputDocument doc, boolean deleted) {
  ...
  public static void setDublinCore(SolrInputDocument doc, OcDublinCore dc) {
    try {
      doc.setField(DUBLINCORE, OcDublinCoreUtil.toCatalog(dc).toXmlString());
    } catch (IOException e) {
      chuck(e);
    }
  }

  public static void setDublinCore(SolrInputDocument doc, DublinCoreCatalog dc) {
    try {
      doc.setField(DUBLINCORE, dc.toXmlString());
    } catch (IOException e) {
      chuck(e);
    }
  }
  ...
  public static OcDublinCore getDublinCore(SolrDocument doc) {
    return convertToOcDublinCore(doc, DUBLINCORE);
  }

  public static Option<OcDublinCore> getSeriesDublinCore(SolrDocument doc) {
    if (StringUtils.isBlank(mkString(doc.get(DUBLINCORE_SERIES)))) {
      return Option.<OcDublinCore>none();
    } else {
      return some(convertToOcDublinCore(doc, DUBLINCORE_SERIES));
    }
  }

  /** Convert a solr field which contains DublinCore XML into an {@link org.opencastproject.schema.OcDublinCore}. */
  private static OcDublinCore convertToOcDublinCore(SolrDocument doc, String field) {
    final String xml = mkString(doc.get(field));
    for (final DublinCoreCatalog dc : DublinCoreUtil.fromXml(xml)) {
      final OcDublinCoreConversion conv = OcDublinCoreUtil.create(dc);
      if (!conv.getConversionErrors().isEmpty()) {
        final String errors = mlist(conv.getConversionErrors()).mkString(",");
        final String dcId = option(dc.getFirst(DublinCore.PROPERTY_IDENTIFIER)).getOrElse("?");
        logger.warn(format("Errors converting DublinCoreCatalog %s into an OcDublinCore: %s", dcId, errors));
      }
      return conv.getDublinCore();
    }
    final String msg = format("Cannot convert field %s of solr document into a DublinCoreCatalog", field);
    logger.error(msg + "\n Field: " + xml);
  ...
  public static <A> Option<A> getFirst(List<DField<A>> fs) {
    return head(fs).map(new Function<DField<A>, A>() {
      @Override
      public A apply(DField<A> f) {
        return f.getValue();
  ...
  /** Return a function that returns the value of the first dfield. */
  public static <A> Function<List<DField<A>>, Option<A>> getFirst() {
    return new Function<List<DField<A>>, Option<A>>() {
      @Override
      public Option<A> apply(List<DField<A>> fs) {
        return getFirst(fs);
      }
    };
  }
