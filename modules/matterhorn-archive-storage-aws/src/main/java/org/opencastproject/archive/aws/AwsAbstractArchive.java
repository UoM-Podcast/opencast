...
package org.opencastproject.archive.aws;

import static org.apache.commons.lang3.exception.ExceptionUtils.getMessage;

import org.opencastproject.archive.api.Version;
import org.opencastproject.archive.aws.persistence.AwsAssetDatabase;
...
public abstract class AwsAbstractArchive extends AbstractRemoteElementStore {
  ...
  protected String buildFilename(StoragePath path, String ext) {
    ...
    return StringUtils.join(new String[] { path.getOrganizationId(), path.getMediaPackageId(),
            path.getVersion().toString(), path.getAssetId() + ext }, "/");
  }

  /**
   * @see org.opencastproject.archive.base.storage.ElementStore#put(StoragePath, Source)
   */
  ...
  public Option<InputStream> get(final StoragePath path) throws ElementStoreException {
    try {
      ...
      return Option.some(getObject(map));

    } catch (ElementStoreException e) {
      throw e;
    } catch (AwsAssetDatabaseException e) {
      throw new ElementStoreException(e);
    }
