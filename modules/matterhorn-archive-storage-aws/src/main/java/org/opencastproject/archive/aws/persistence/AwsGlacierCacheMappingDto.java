...
@Table(name = "mh_aws_cache_mapping")
@NamedQueries({
...
public final class AwsGlacierCacheMappingDto {
  ...
  public static AwsGlacierCacheMappingDto storeMapping(EntityManager em, StoragePath path) throws
    ...
    AwsGlacierCacheMappingDto mapDto = new AwsGlacierCacheMappingDto(path.getOrganizationId(), path.getMediaPackageId(),
            path.getAssetId(), Long.valueOf(path.getVersion().toString()));
    ...
    try {
      ...
      throw new AwsAssetDatabaseException(format("Could not store mapping for path %s", path), e);
    } finally {
      em.close();
    }
  }
   ...
   * Deletes a mapping.
   */
  public static void deleteMapping(EntityManager em, StoragePath path) throws AwsAssetDatabaseException {
